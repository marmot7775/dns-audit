...
def build_dmarc_roadmap(raw_dmarc: Dict, raw_spf: Dict, raw_dkim: Dict,
    ...
    steps.append({
        ...
        "status": step3_status,
        "dns_record": f"v=DMARC1; p=none; rua={rua_addr}" if not step3_complete else None,
    ...
    steps.append({
        ...
        "status": step5_status,
        "dns_record": f"v=DMARC1; p=quarantine; t=y; rua={rua_addr}" if not step5_complete else None,
    ...
    steps.append({
        ...
        "status": step6_status,
        "dns_record": f"v=DMARC1; p=quarantine; rua={rua_addr}" if not step6_complete else None,
        "prerequisites": None,
        "warnings": None,
        "estimated_duration": "1-2 weeks",
    })
    ...
    steps.append({
        ...
        "status": step7_status,
        "dns_record": f"v=DMARC1; p=reject; rua={rua_addr}" if not step7_complete else None,
        "prerequisites": None,
        "warnings": None,
        "estimated_duration": "Immediate",
    })
